...
local COMMANDER = {
  [UnitDefNames["clancom"].id] = true,
  [UnitDefNames["iscom"].id] = true,
}
local comStrings = {
  [1] = "clancom",
  [2] = "iscom",
...
function gadget:UnitCreated(unitID, unitDefID, unitTeam)
  if (not validCom) and COMMANDER[unitDefID] and (not excludeTeams[unitTeam]) then
  ...
  if preStart and startPoints[id] then
    ...
    elseif (msg:find("coop_factionchange",1,true)) then
      ...
      if id == (playerID * 1) then
        if startPoints[id].commander == "iscom" then
          startPoints[id].commander = "clancom"
        else
          startPoints[id].commander = "iscom"
        end
