...
for featureDefID, defs in pairs(FeatureDefs) do
  if string.find(defs.tooltip, 'Wreckage') or string.find(defs.tooltip, 'Shards') or string.find(defs.tooltip, 'Rubble') or string.find(defs.tooltip, 'Debris') then
  ...
  if not cegList[featureID] then
    ...
    if featureDefs then
      if (featureDefs == -1) then
        ...
        featureList[featureDefID] = {minX = (defs.minx * 0.6), maxX = (defs.maxx * 0.6), minZ = (defs.minz * 0.6), maxZ = (defs.maxz * 0.6), y = (defs.maxy * 0.5)}
