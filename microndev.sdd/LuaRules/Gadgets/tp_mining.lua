...
local debug = false
-----config-----
local miner_name = {"skminer","skminerdes"}		--the unit used for mining
local resource_name = {}--{"bminerals","bmeteorimpact","bmeteorimpact_big"}		--the stuff that gets mined
local dropoff_name = {"skrefinery","nwdcom","nwdcomdes","skrefinerydes"}	--where the miners bring the resources to
local dropoff_distance = 75 --how near do miners have to get to a dropoff to drop their cargo? (this value is added to unitRadius)
local maxcargo = 500			--how much a miner can carry before having to return to a drop off
...
function gadget:UnitDestroyed(unitID, unitDefID, teamID, attackerID)
	...
	if (is_resource_type (unitDefID) and is_miner(attackerID)) then
		...
		miners[attackerID].cargo = miners[attackerID].cargo + 100
			...
			if (SYNCED.miners[i].alive == true and i ~= nil and SYNCED.miners[i] ~=nil) then
				...
				if (unitally == localplayerally) then --only mark our own miners
					...
					if (x ~= nil and y ~= nil and z ~= nil) then
						...
						gl.Color(1, 1, 0, 1)
						gl.DrawGroundCircle (x,y,z, SYNCED.miners[i].cargo, 1)  --growing triangle shows cargo status
						...
						if (Spring.ValidUnitID  (SYNCED.miners[i].last_mined_id)) then
							...
							gl.Color(0, 0.8, 0, 1)
							gl.DrawGroundCircle (x,y,z, 1, 1)  --growing triangle shows cargo status							
						end
					end
				end
			end
