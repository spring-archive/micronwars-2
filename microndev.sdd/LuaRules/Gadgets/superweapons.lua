...
--------------------------------------------------------------------------------------------------------------------------END VALIDATION			
	local function FindNewSpot(MinZ,MinX)
    	RandomX = RandomX + math_random(768)
		RandomZ = RandomZ + math_random(768)
...
---------------------------------------------------------------------- Placement Solver, AncientMetal
	for MinZ = 3000,MaxSizeZ,9000 do
		for MinX = 3000,MaxSizeX,9000 do
			RandomFacing = math_random(0,3)

			RandomX = math_random (7000)
			RandomZ = math_random (7000)
			RandomSizeX = MinX + RandomX
			RandomSizeZ = MinZ + RandomZ
			...
			if Validation == 1 then
				MetalCreated = MetalCreated + 1
				CreateUnit("ioncannon",CenterX,0,CenterZ,RandomFacing,Spring.GetGaiaTeamID())
...
---------------------------------------------------------------------- Placement Solver, AncientPower	
	for MinZ = 5000,MaxSizeZ,10000 do
		for MinX = 5000,MaxSizeX,15000 do
			RandomFacing = math_random (0,3)
			...
			if Validation == 1 then
				PowerCreated = PowerCreated + 1
				CreateUnit("bunker",CenterX,0,CenterZ,RandomFacing,Spring.GetGaiaTeamID())
...
--[[	if f == 99 then
	...
	if f == 100 then
		...
		gadgetHandler.RemoveGadget("Superweapons Spawn")
	end
---------------------------------------------------------------------- END REPORTING
end
-------------------------------------------------------------------------------------------END SYNC
