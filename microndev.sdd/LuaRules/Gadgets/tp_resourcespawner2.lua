...
---config---
local resource_name = "bigcrystal" 
					--starting resource to be found on the map
local mapconfig_fn = Game.mapName .. "_res2.lua"		--name of map config files
...
if (VFS.FileExists(mapconfig_fn)) then 
	Spring.Echo ("tp_resourcespawner2: found" .. mapconfig_fn .." in mod root folder or map")
	gamesettings = VFS.Include(mapconfig_fn)
else
	if (VFS.FileExists("mapconfigs\\" .. mapconfig_fn)) then 
		Spring.Echo ("tp_resourcespawner2: found " .. mapconfig_fn .. " in mod mapconfigs folder")
		gamesettings = VFS.Include("mapconfigs\\" .. mapconfig_fn)
	else
		Spring.Echo ("tp_resourcespawner2: " .. mapconfig_fn .. " not found at all")
	end
end
...
local function SpawnResource (sx,sz)
	if (sx == nil or sz == nil) then
		Spring.Echo ("tp_resourcespawner2: sx or sz was NIL in SpawnResource (sx,sz)")
		return
	end
...
local mspots = 0
	...
	Spring.Echo ("tp_resourcespawner2: placing resources by metal map")
	...
	Spring.Echo ("tp_resourcespawner2: placed " .. mmrescounter .. " resources by metal map")
end

local function PutResourcesOnMap ()
	if (gamesettings == nil) then 
		Spring.Echo ("tp_resourcespawner2: gamesettings==nil") 
		PutResoucesOnMetal ()
		return 
	end
	Spring.Echo ("tp_resourcespawner2: ----PutResourcesOnMap ()----")
		...
		if (maxMetal) then
			if (maxMetal > 0) then
				...
				if (featurehandling=="replace") then 
					...
					Spring.Echo ("tp_resourcespawner2: feature replaced")
				end
				if (featurehandling=="remove") then 				
					Spring.DestroyFeature (all_features[i])
					Spring.Echo ("tp_resourcespawner2: feature removed")
				end
			end
		end
	end
...
function gadget:GameStart()
	Spring.Echo ("tp_resourcespawner2: Hey I am the resource spawn gadget")
	PutResourcesOnMap ()
	FeatureHandling ()
...
Spring.Echo ("tp_resourcespawner2: Game.mapName=" .. Game.mapName)
end
